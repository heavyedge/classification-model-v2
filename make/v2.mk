...
DATASETS_v2 := $(if $(filter 1,$(HEAVYEDGE_TEST_MODE)),dataset5,$(shell ls -d _data/v1/profiles/mean_profiles/dataset* | xargs -n 1 basename))
PROFILES_v2 = $(shell ls _data/v1/profiles/mean_profiles/$(1)/*.h5)
...
test-v2: $(foreach method,$(CALIBRATION_METHODS_v2),models/v2/classifiers/minirocket.$(method).pkl)
	...
	for model in $^; do
		echo "Testing $$model.."
		heavyedge --log-level=INFO classify-predict _data/v1/profiles/mean_profiles/dataset5/001.h5 $$model -o $$out
	done
...
_temp/v2/knees.csv: $(foreach dataset, $(DATASETS_v2), _data/v1/labels/$(dataset)/knees.csv)
	mkdir -p $(@D)
	python3 -c "import pandas as pd; dfs = [pd.read_csv(path) for path in '$^'.split()]; pd.concat(dfs)[['Type']].to_csv('$@', index=False)"

_temp/v2/canonical.csv: $(foreach dataset, $(DATASETS_v2), _data/v1/labels/$(dataset)/canonical.csv)
	mkdir -p $(@D)
	python3 -c "import pandas as pd; dfs = [pd.read_csv(path) for path in '$^'.split()]; pd.concat(dfs)[['Type']].to_csv('$@', index=False)"
...
_data/v1/profiles/mean_profiles/dataset1/013.h5 \
_data/v1/profiles/mean_profiles/dataset5/013.h5 \
_data/v1/profiles/mean_profiles/dataset5/033.h5 \
_data/v1/profiles/mean_profiles/dataset5/016.h5 \
_data/v1/profiles/mean_profiles/dataset2/017.h5 \
_data/v1/profiles/mean_profiles/dataset2/356.h5 \
_data/v1/profiles/mean_profiles/dataset1/027.h5
	mkdir -p $(@D)
	heavyedge merge $^ -o $@
